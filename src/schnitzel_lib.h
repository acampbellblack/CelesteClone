...
// Used to get the timestamp of files
#include <sys/stat.h>
...
#define SM_WARN(msg, ...) _Log("[TRACE]", msg, TEXT_COLOR_YELLOW, ##__VA_ARGS__);
#define SM_ERROR(msg, ...) _Log("[TRACE]", msg, TEXT_COLOR_RED, ##__VA_ARGS__);
...
char* bump_alloc(BumpAllocator* bumpAllocator, size_t size)
{
    ...
    size_t allignedSize = (size + 7) & ~7; // Ensure first 4 bits are 0
...
char* read_file(char* filePath, int* fileSize, char* buffer)
{
    ...
    *fileSize = ftell(file);
    ...
    memset(buffer, 0, *fileSize + 1); // ensure null terminated
