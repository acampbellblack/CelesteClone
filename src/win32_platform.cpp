...
bool platform_create_window(int width, int height, char* title)
{
    ...
    wc.hCursor = LoadCursor(instance, IDC_ARROW);   // Decide the look of the cursor
    ...
    // Fake window initializing OpenGL
    {
        ...
        if (!fakeDC)
        {
            SM_ASSERT(false, "Failed to get fake DC");
            return false;
        }
        ...
        {
            SM_ASSERT(false, "Failed to load wgl functions");
            return false;
        }
...
void platform_update_window()
{
    ...
    while (PeekMessageA(&message, window, 0, 0, PM_REMOVE))
    {
        ...
        DispatchMessageA(&message); // Calls the specified callback
    }
}

// MARK: _load_gl_function
...
// MARK: _fill_keycode_lookup_table
